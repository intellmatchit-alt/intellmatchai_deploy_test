...
import {
  ...
  createSAVideoCategory,
  updateSAVideoCategory,
  deleteSAVideoCategory,
  type SAVideoCategory,
} from '@/lib/api/superadmin';

const emptyCategory: Partial<SAVideoCategory> = {
  name: '',
  nameAr: '',
  slug: '',
  sortOrder: 0,
...
export default function SAVideoCategoriesPage() {
  const [categories, setCategories] = useState<SAVideoCategory[]>([]);
  const [isLoading, setIsLoading] = useState(true);
  const [editing, setEditing] = useState<Partial<SAVideoCategory> | null>(null);
  const [saving, setSaving] = useState(false);

  const load = useCallback(async () => {
    setIsLoading(true);
    try {
      setCategories(await getSAVideoCategories());
    } catch { /* ignore */ }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (editing.id) {
        await updateSAVideoCategory(editing.id, payload);
      } else {
        await createSAVideoCategory(payload);
      }
      setEditing(null);
      load();
    } catch (err: any) {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('Delete this video category?')) return;
    try {
      await deleteSAVideoCategory(id);
      load();
    } catch (err: any) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-xl font-semibold text-white font-bold">Video Categories</h2>
          <p className="text-sm text-white font-bold mt-1">Manage categories for the video gallery</p>
        </div>
        <button
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#2a2a35]">
              ...
              <th className="text-left px-4 py-3 text-white font-bold font-medium">Slug</th>
              <th className="text-center px-4 py-3 text-white font-bold font-medium">Sort</th>
          ...
          <tbody>
            {categories.map((cat) => (
              <tr key={cat.id} className="border-b border-[#2a2a35] last:border-b-0 hover:bg-[#16161e] transition-colors">
                <td className="px-4 py-3 text-white font-bold font-medium">{cat.name}</td>
                <td className="px-4 py-3 text-white font-bold" dir="rtl">{cat.nameAr || '—'}</td>
                <td className="px-4 py-3 text-white font-bold font-mono text-xs">{cat.slug}</td>
                <td className="px-4 py-3 text-center text-white font-bold">{cat.sortOrder}</td>
                <td className="px-4 py-3 text-center">
                  <span className={`inline-block w-2.5 h-2.5 rounded-full ${cat.isActive ? 'bg-green-400' : 'bg-gray-500'}`} />
                </td>
                <td className="px-4 py-3 text-center text-white font-bold">{cat._count?.videos ?? 0}</td>
            ...
            {categories.length === 0 && (
              <tr>
                <td colSpan={7} className="text-center py-12 text-white font-bold">
                  No video categories yet. Create one to get started.
                </td>
              </tr>
            )}
          </tbody>
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4" onClick={() => setEditing(null)}>
          <div className="bg-[#16161e] rounded-2xl p-6 border border-[#2a2a35] w-full max-w-md space-y-4" onClick={(e) => e.stopPropagation()}>
            <div className="flex items-center justify-between">
              <h3 className="text-lg font-semibold text-white font-bold">{editing.id ? 'Edit Category' : 'Create Category'}</h3>
            ...
            {[
              { key: 'name', label: 'Name', type: 'text', placeholder: 'e.g. Tutorials' },
              { key: 'nameAr', label: 'Name (Arabic)', type: 'text', placeholder: 'e.g. دروس تعليمية' },
              { key: 'slug', label: 'Slug (auto-generated if empty)', type: 'text', placeholder: 'e.g. tutorials' },
