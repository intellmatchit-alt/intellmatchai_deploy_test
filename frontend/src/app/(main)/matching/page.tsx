...
const matchingOptions = [
  ...
  },
  {
    ...
    fallback: "Pitch",
    description:
      "Upload pitch decks and find the right people to connect with.",
  ...
  },
  {
    ...
    description:
      "Describe what you need and let AI find the best contacts for you.",
      ...
      try {
        ...
        setCounts({
          projects: {
            ...
            matches: projectsList.reduce(
              (sum: number, p: any) => sum + (p.matchCount || 0),
              0,
            ),
          },
          pitch: {
            items: pitchList.length,
            matches: pitchList.reduce(
              (sum: number, p: any) => sum + (p.matchCount || 0),
              0,
            ),
          },
          deals: {
            ...
            matches: dealsList.reduce(
              (sum: number, d: any) => sum + (d.matchCount || 0),
              0,
            ),
          },
          opportunities: {
            items: oppsList.length,
            matches: oppsList.reduce(
              (sum: number, o: any) => sum + (o.matchCount || 0),
              0,
            ),
          },
        });
    ...
    <div className="space-y-6 animate-fade-in pb-6">
      ...
      <div className="text-center pt-2">
        ...
        <p className="text-white font-bold mt-1 max-w-xs mx-auto text-sm">
          {t.matchingHub?.subtitle ||
            "AI-powered tools to find the right people for your goals"}
        </p>
      </div>
      ...
      <div className="grid grid-cols-2 gap-4 px-1">
        {matchingOptions.map((option) => {
          ...
          return (
            <Link key={option.href} href={option.href} className="group h-full">
              <div
              ...
              >
                ...
                <p className="text-sm text-white font-bold leading-relaxed line-clamp-2 flex-1">
                  {(t.matchingHub?.cards as any)?.[option.labelKey] ||
                    option.description}
                </p>
                ...
                <div className="relative mt-3 flex items-center gap-2">
                  {loading ? (
                    ...
                    <>
                      <div
                        className={`flex items-center gap-1.5 px-2.5 py-1 rounded-full border ${option.matchBg}`}
                      >
                        <option.Icon
                          className={`w-4 h-4 ${option.matchText}`}
                        />
                        <span
                          className={`text-xs font-bold ${option.matchText}`}
                        >
                          {cardCounts.items}
                        </span>
                      </div>
                ...
                <div className="relative mt-3 flex items-center gap-2 text-sm">
                  <span className="px-3 py-1.5 rounded-lg bg-emerald-400 text-[#042820] font-bold text-xs">
                    {t.matchingHub?.open || "Open"}
                  </span>
        ...
        <div className="bg-gradient-to-r from-emerald-500/[0.08] to-teal-500/[0.08] border border-emerald-500/20 rounded-2xl p-4 flex items-center gap-4 transition-all hover:border-emerald-400/40 hover:bg-emerald-500/[0.12] active:scale-[0.98]">
          ...
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-white text-sm">
              {t.matchingHub?.viewAllMatches || "View All Matches"}
            </h3>
            <p className="text-xs text-white font-bold">
              {t.matchingHub?.viewAllMatchesSub ||
                "See all AI matches across projects, deals, pitch, and jobs"}
            </p>
          </div>
