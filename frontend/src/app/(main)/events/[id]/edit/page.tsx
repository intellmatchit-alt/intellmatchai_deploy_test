 ...
 * Edit Event Page
 *
 * Form to edit an existing networking event.
 */
...
import { getEvent, updateEvent, uploadEventThumbnail } from '@/lib/api/events';
...
export default function EditEventPage() {
    ...
    const fetchEvent = async () => {
      try {
        ...
        if (event.thumbnailUrl) {
          ...
          setOriginalThumbnail(event.thumbnailUrl);
        }
      } catch (error: any) {
        toast({ title: 'Error', description: error.message || 'Failed to load event', variant: 'error' });
        router.push('/events');
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (thumbnailFile) {
        try {
          await uploadEventThumbnail(eventId, thumbnailFile);
        } catch (err) {
          console.error('Failed to upload thumbnail:', err);
        }
      }

      toast({ title: 'Event updated!', description: 'Your changes have been saved', variant: 'success' });
  ...
  if (loadingEvent) {
    return (
      <div className="min-h-screen bg-gradient-to-b from-th-bg via-th-bg-s to-th-bg p-4 md:p-6 flex items-center justify-center">
        <div className="flex items-center gap-3 text-th-text">
          <ArrowSync24Regular className="w-6 h-6 animate-spin" />
    ...
    <div className="min-h-screen bg-gradient-to-b from-th-bg via-th-bg-s to-th-bg p-4 md:p-6">
      <div className="max-w-2xl mx-auto">
        ...
        <div className="flex items-center gap-4 mb-6">
          ...
          <div>
            <h1 className="text-2xl font-bold text-th-text">Edit Event</h1>
            <p className="text-th-text-t mt-1">Update your event details</p>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="bg-th-surface backdrop-blur-sm border border-th-border rounded-xl p-6 space-y-5">
            {/* Event Status */}
            <div className="flex items-center justify-between p-3 bg-th-surface rounded-lg border border-th-border">
              <div>
                <p className="text-th-text font-medium">Event Status</p>
                <p className="text-sm text-th-text-t">
                  {formData.isActive ? 'Active - accepting registrations' : 'Inactive - registration closed'}
                </p>
              </div>
              <button
                type="button"
            ...
            <div>
              ...
              <div className="relative">
                <input
                  ...
                  className={`w-full px-4 py-3 bg-th-surface border rounded-lg text-th-text placeholder-th-text-t focus:outline-none focus:ring-2 focus:ring-emerald-500 ${
            ...
            <div>
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-4 py-3 bg-th-surface border border-th-border rounded-lg text-th-text placeholder-th-text-t focus:outline-none focus:ring-2 focus:ring-emerald-500"
                  maxLength={500}
                />
                <button
                  type="button"
            ...
            {showMapPicker && (
              <div className="space-y-2">
                <input
                  ...
                  className="w-full px-4 py-2 bg-th-surface border border-th-border rounded-lg text-th-text placeholder-th-text-t focus:outline-none focus:ring-2 focus:ring-emerald-500"
                />
                <div
                  ref={mapRef}
              ...
              {thumbnailPreview ? (
                <div className="relative">
                  ...
                  <div className="absolute top-2 right-2 flex gap-2">
                    <label className="p-2 bg-emerald-500 rounded-full text-th-text hover:bg-emerald-600 cursor-pointer">
            ...
            <div>
              ...
              <textarea
                ...
                rows={3}
                className="w-full px-4 py-3 bg-th-surface border border-th-border rounded-lg text-th-text placeholder-th-text-t focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
                maxLength={500}
              />
        ...
        {(thumbnailPreview || formData.welcomeMessage) && (
          <div className="mt-6 p-4 bg-th-surface border border-th-border rounded-xl">
