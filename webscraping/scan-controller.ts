...
const createContactFromScanUseCase = new CreateContactFromScanUseCase(
  contactRepository,
);
...
export class ScanController {
   ...
   */
  private async extractFieldsWithGPT(
    rawText: string,
  ): Promise<GPTExtractedFields | null> {
...
Use null for any field you cannot determine. Be smart about fixing OCR errors!`;
    ...
    try {
            ...
            messages: [
              {
                role: "system",
                content:
                  "You are a business card OCR expert. Always respond with valid JSON only.",
              },
      ...
      if (!response.ok) {
        ...
        logger.error("GPT field extraction API error", {
   ...
   */
  private buildSearchQueries(data: {
    name: string;
    company?: string;
    jobTitle?: string;
    email?: string;
    ...
    if (data.company) {
      queries.push(
        `"${name}" "${data.company}" CEO OR founder OR director OR manager`,
      );
    }

    // Query 7: News and recent mentions
    queries.push(
      `"${name}" ${data.company || ""} news OR interview OR announcement`.trim(),
    );
  ...
  private async searchWithGoogle(
    ...
    try {
      ...
      const searchPromises = queries
        .slice(0, 5)
        .map(async (query): Promise<GoogleCSEResponse> => {
  ...
  private async analyzeGoogleResults(
    ...
    const detectedLinkedIn =
      linkedInUrl || googleUrls.find((u) => u.includes("linkedin.com/in/"));
...
1. **BIO**: Write 3-4 compelling sentences about this person.
   - PRIORITY: Use LinkedIn about/summary section if found in search results
   - Include their current role and responsibilities
   - What their company does (infer from name: "${cleanCompany || "their company"}")
   - Their expertise and experience level
   ...
   - Write it as a clean, professional narrative

2. **SECTORS**: What industries does "${cleanCompany || "their company"}" operate in?
...
3. **SKILLS**: Extract skills for ${cleanTitle || "professional"} at ${cleanCompany || "this company"}
...
4. **INTERESTS**: What professional topics would they follow?
   - Industry trends
   - Technologies
   - Business topics
   - Think: What conferences would they attend?

5. **LINKEDIN**: Extract the EXACT LinkedIn profile URL if found in the URLs above.
   Look for URLs containing "linkedin.com/in/" - that's their personal profile.

6. **LOCATION**: ${cleanLocation ? `Confirm: ${cleanLocation}` : "Extract from search results or infer from company/email domain"}

=== RESPOND WITH THIS JSON ONLY ===
{
  "bio": "3-4 sentence clean professional bio. NO URLs, NO citations, NO employee counts, NO markdown links. Just a clean narrative about their role, company, expertise. ${cleanLocation ? `Mention ${cleanLocation}.` : ""}",
...
IMPORTANT:
- Make the bio IMPRESSIVE - like a LinkedIn summary
- BIO MUST BE CLEAN: No URLs, no citations like ([source](url)), no employee counts like "leads X employees"
...
- If title has "CTO/Engineer" → include Technical Architecture, Engineering, Innovation`;
    ...
    try {
      ...
      logger.info("Analyzing Google results with GPT", {
        name: data.name,
        snippetsCount: googleSnippets.length,
      });
            ...
            messages: [
              {
                role: "system",
                content:
                  'You are an elite professional researcher who creates impressive, detailed profiles. You analyze business cards and search results to build comprehensive professional profiles that make users say "Wow, how did it know all this about me!" Always respond with valid JSON only. Be specific, detailed, and impressive.',
              },
              { role: "user", content: prompt },
            ],
            temperature: 0.4,
      ...
      if (!response.ok) {
        logger.warn("Google results analysis API error", {
          status: response.status,
        });
        return null;
      }
      ...
      // Clean the bio - remove any links, citations, or employee counts that slipped through
      let cleanBio = parsed.bio || "";
      // Remove markdown links like [text](url)
      cleanBio = cleanBio.replace(/\[([^\]]*)\]\([^)]*\)/g, "$1");
      // Remove parenthetical citations like (website.com) or (source)
      cleanBio = cleanBio.replace(
        /\s*\([^)]*\.(com|co|org|net|io)[^)]*\)/gi,
        "",
      );
      // Remove "leads a team of X employees" variations
      cleanBio = cleanBio.replace(
        /\s*(He|She|They)\s+leads?\s+a\s+team\s+of\s+[\d–-]+\s*(employees?|people|staff)?\.?/gi,
        "",
      );
      // Remove standalone URLs
      cleanBio = cleanBio.replace(/https?:\/\/[^\s)]+/g, "");
      ...
      return {
        ...
        sectors: Array.isArray(parsed.sectors)
          ? parsed.sectors.slice(0, 5)
          : [],
        interests: Array.isArray(parsed.interests)
          ? parsed.interests.slice(0, 6)
          : [],
        linkedInUrl: finalLinkedIn,
        location: finalLocation,
        sources: googleUrls.slice(0, 5),
        confidence: finalLinkedIn
          ? "high"
          : googleSnippets.length >= 3
            ? "high"
            : "medium",
      };
    } catch (error) {
  ...
  private async searchWithOpenAI(
    data: { name: string; company?: string; jobTitle?: string },
...
{
  "bio": "Write 3-4 sentences using REAL facts from LinkedIn/Wikipedia. Include: their current role at ${cleanCompany || "their company"}, what they do, their expertise, achievements. NO URLs, NO citations, NO employee counts.",
...
2. Bio must be from REAL data found online, not generic text
3. Skills should be from LinkedIn "Skills" section if found
4. Sectors must match what ${cleanCompany || "their company"} actually does
5. NO fabricated information - only use what you actually find
6. Bio MUST be clean: no URLs, no citations like [source], no markdown links
7. If you find Wikipedia, USE that authoritative information
8. Current data matters - prefer recent information (2024-2025)
...
RESPOND WITH ONLY THE JSON:`;
    ...
    try {
      ...
      logger.info("OpenAI raw response status", {
        status: response.status,
        length: responseText.length,
      });

      if (!response.ok) {
        logger.error("OpenAI web search API error", {
          status: response.status,
      ...
      const sources = Array.isArray(parsed.sources)
        ? parsed.sources.filter((s: string) => s?.startsWith("http"))
        : [];
  ...
  private async performAIAnalysis(data: {
    ...
    const googleContext =
      googleResults.snippets.length > 0
    ...
    const googleLinkedIn = googleResults.urls.find((u) =>
    ...
    if (openaiHasData && openaiResult) {
      ...
      logger.info(
        "OpenAI web_search returned limited results, trying Google results analysis",
      );
      ...
      if (
        googleAnalysis &&
        (googleAnalysis.bio ||
          googleAnalysis.sectors.length > 0 ||
          googleAnalysis.skills.length > 0)
      ) {
        ...
        logger.info(
          "Using keyword-based profile (all AI analysis returned limited results)",
        );
        ...
        const searchText =
          `${cleanCompany || ""} ${cleanTitle || ""}`.toLowerCase();
        ...
        if (
          ...
          if (!detectedSkills.includes("Technology"))
            detectedSkills.push("Technology Solutions");
          if (!detectedSkills.includes("Digital Transformation"))
        ...
        try {
          ...
          if (gptFields) {
            ...
            finalFields = {
              ...
              linkedInUrl:
                gptFields.linkedInUrl || ocrResult.fields.linkedInUrl,
          ...
          logger.warn("GPT extraction failed, using OCR fields", {
            error: gptError,
          });
        }
      }
  ...
  async scanCardStream(
    ...
    try {
      ...
      res.write(
        `data: ${JSON.stringify({ stage: "result", progress: 100, result })}\n\n`,
      );
      res.end();
    } catch (error) {
      // Send error via SSE
      res.write(
        `data: ${JSON.stringify({ stage: "error", error: (error as Error).message })}\n\n`,
      );
      res.end();
    }
  }
  ...
  async confirmScan(
    ...
    try {
      ...
      const contact = await createContactFromScanUseCase.execute(
        req.user.userId,
        req.body,
      );
