...
const INVESTOR_PATTERN =
  /\b(investor|venture\s*capital|vc\s|angel|fund\s*manager|portfolio|capital|investment\s*(director|manager|analyst))\b/i;
const ADVISOR_PATTERN =
  /\b(advisor|mentor|board\s*(member|director)|consultant|coach|strategist)\b/i;
const COFOUNDER_PATTERN = /\b(co-?founder|founding|cto|ceo|coo|cmo|chief)\b/i;
const TALENT_PATTERN =
  /\b(developer|engineer|designer|architect|analyst|scientist|manager|specialist|coordinator|lead\b)/i;
const SERVICE_PATTERN =
  /\b(agency|firm|provider|studio|consultancy|solutions|services\s*(company|llc|inc)?)\b/i;

export function inferCounterpartType(
  jobTitle?: string,
  bio?: string,
  company?: string,
): CounterpartType {
...
export function inferExecutionTrack(
  jobTitle?: string,
  bio?: string,
): ExecutionTrack {
  const text = [jobTitle, bio].filter(Boolean).join(" ").toLowerCase();
  if (/investor|fund|capital|vc|angel/.test(text))
    return ExecutionTrack.FINANCIAL;
  if (/advisor|mentor|board|consultant|coach/.test(text))
    return ExecutionTrack.ADVISORY;
  if (/ceo|coo|director|vp|head\s+of|manager/.test(text))
    return ExecutionTrack.STRATEGIC;
  if (/cto|engineer|developer|architect|devops|data/.test(text))
    return ExecutionTrack.TECHNICAL;
  if (/operations|logistics|supply|procurement/.test(text))
    return ExecutionTrack.OPERATIONAL;
...
export function inferSeniority(jobTitle?: string): SeniorityLevel | undefined {
  ...
  if (/\b(chief|c-level|ceo|cto|coo|cfo|cmo|president)\b/.test(t))
    return SeniorityLevel.C_LEVEL;
  if (/\b(vp|vice\s*president|evp|svp)\b/.test(t))
  ...
  if (/\b(junior|jr\.?|intern|associate)\b/.test(t))
...
function extractKeywords(
  ...
  return [
    ...new Set(tokens.filter((t) => t.length > 2 && !stopWords.has(t))),
  ].slice(0, 30);
}
...
export function buildProviderFromUser(
  ...
  const counterpartType = inferCounterpartType(
  ...
  return {
    ...
    executionTrack: inferExecutionTrack(
      user.jobTitle || undefined,
      user.bio || undefined,
    ),
    ...
    dataQualityScore: calculateDataQuality({
      ...
      interests,
      hobbies: user.userHobbies?.map((h) => h.hobby.name),
...
export function buildProviderFromContact(
  ...
  return {
    ...
    dataQualityScore: calculateDataQuality({
      ...
      interests,
      hobbies: contact.contactHobbies?.map((h) => h.hobby.name),
