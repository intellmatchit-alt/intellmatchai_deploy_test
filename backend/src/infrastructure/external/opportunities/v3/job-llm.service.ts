...
import {
  ...
  HelperAIValidationItem,
  HelperAIValidationRequest,
...
Respond ONLY with valid JSON:
{
  ...
  // ==========================================================================
  // HELPER-FLOW AI VALIDATION (Phase 4)
  //
  // Answers "Can this person realistically help the candidate reach
  // relevant job opportunities?" — NOT "is this person suitable for a job".
  // Bounded by AI_MAX_SCORE_ADJUSTMENT and never overrides hard-filter FAIL
  // (the service layer enforces that — the LLM never sees FAILed helpers).
  // ==========================================================================

  async validateHelperMatches(
    request: HelperAIValidationRequest,
  ): Promise<HelperAIValidationItem[]> {
    if (!this.primary) {
      return this.fallbackHelperValidations(request.helpers);
    }

    const prompt = this.buildHelperValidationPrompt(request);
    try {
      const raw = await this.callWithCascade(prompt);
      return this.parseHelperValidation(
        raw,
        request.deterministicScores,
        request.helpers,
      );
    } catch (e) {
      console.error("[JobLLM] Helper validation failed", e);
      return this.fallbackHelperValidations(request.helpers);
    }
  }

  private buildHelperValidationPrompt(req: HelperAIValidationRequest): string {
    const cand = req.candidate as any;
    const candSkills = Array.isArray(cand?.skills)
      ? cand.skills.slice(0, 10).join(", ")
      : "none";
    const candIndustries = Array.isArray(cand?.industries)
      ? cand.industries.slice(0, 5).join(", ")
      : "none";
    const desiredEmployment = Array.isArray(cand?.desiredEmploymentType)
      ? cand.desiredEmploymentType.join(", ")
      : "any";
    const desiredWorkMode = Array.isArray(cand?.desiredWorkMode)
      ? cand.desiredWorkMode.join(", ")
      : "any";
    const summary = (cand?.profileSummaryPreferences || "").slice(0, 600);

    const helperSummaries = req.helpers
      .map((h, i) => {
        const skills = h.skills?.slice(0, 8).join(", ") || "none";
        const sectors = h.sectors?.slice(0, 5).join(", ") || "none";
        const network = `degree=${h.network.degree}${h.network.sameOrganization ? ", same-org" : ""}`;
        return `${i}. [${h.id}] ${h.fullName} — ${h.jobTitle || "Unknown title"}${h.company ? ` @ ${h.company}` : ""} | helperType=${h.helperType} | ${network} | Skills: ${skills} | Sectors: ${sectors}`;
      })
      .join("\n");

    return `You are evaluating whether each helper can realistically help a job-seeking candidate reach relevant opportunities.

This is NOT a candidate-job-fit check. The helper does NOT need to be a great candidate themselves. Ask only:
"Can this person plausibly REFER, INTRODUCE, ADVISE, CONNECT, REVIEW, or GUIDE the candidate toward relevant job opportunities?"

CANDIDATE (job-seeker):
- Title / target role: ${cand?.title || "Unknown"}
- Role area: ${cand?.roleArea || "Unknown"}
- Seniority: ${cand?.seniority || "Unknown"}
- Years of experience: ${cand?.yearsOfExperience ?? "Unknown"}
- Desired employment: ${desiredEmployment}
- Desired work mode: ${desiredWorkMode}
- Industries of interest: ${candIndustries}
- Skills: ${candSkills}
- Profile summary / preferences: ${summary || "—"}

HELPERS (already pre-filtered to be in network and reachable):
${helperSummaries}

RULES:
- Adjust scores by at most ±${AI_MAX_SCORE_ADJUSTMENT} points from the deterministic baseline.
- Be conservative when uncertain. Favour under-adjustment.
- Strong adjustments require concrete evidence in the helper's title, company, or skills.
- A non-recruiter senior operator can still be a useful warm-introducer or advisor — do not penalise them for not being a recruiter.
- A recruiter or hiring decision-maker in a relevant industry should typically score higher.
- A helper with no clear connection to the candidate's industry, role, or company access should typically score lower.
- Never invent evidence. Never assume facts not present in the prompt.
- Provide concrete, helper-framed reasoning ("can introduce you to engineering managers in fintech"), not candidate-fit reasoning ("would be a good engineer").
- For likelyHelpType, choose ONE of: refer, introduce, advise, connect, review, guide.
...
Respond ONLY with valid JSON in this exact shape:
{
  "validations": [
    {
      "index": 0,
      "helperId": "<exact id shown in brackets>",
      "originalScore": 53,
      "adjustedScore": 60,
      "confidence": 0.7,
      "reasoning": "Concrete helper-framed explanation grounded in evidence shown above.",
      "greenFlags": ["..."],
      "redFlags": ["..."],
      "likelyHelpType": "introduce"
    }
  ]
}`;
  }

  private parseHelperValidation(
    raw: string,
    origScores: number[],
    helpers: HelperAIValidationRequest["helpers"],
  ): HelperAIValidationItem[] {
    try {
      const json = raw.match(/\{[\s\S]*\}/)?.[0];
      if (!json) return this.fallbackHelperValidations(helpers);
      const parsed = JSON.parse(json);
      const items: HelperAIValidationItem[] = [];
      for (const v of parsed.validations || []) {
        const idx = typeof v.index === "number" ? v.index : -1;
        if (idx < 0 || idx >= helpers.length) continue;
        const orig = origScores[idx] || 0;
        let adj = typeof v.adjustedScore === "number" ? v.adjustedScore : orig;
        const diff = adj - orig;
        if (Math.abs(diff) > AI_MAX_SCORE_ADJUSTMENT) {
        ...
        items.push({
          ...
          greenFlags: Array.isArray(v.greenFlags)
            ...
            : [],
          likelyHelpType:
            typeof v.likelyHelpType === "string"
              ? v.likelyHelpType.toLowerCase()
              : null,
        });
      }
      return items;
    } catch {
      return this.fallbackHelperValidations(helpers);
    }
  }

  private fallbackHelperValidations(
    helpers: HelperAIValidationRequest["helpers"],
  ): HelperAIValidationItem[] {
    return helpers.map((h) => ({
      ...
      adjustedScore: 0,
      confidence: 0,
      reasoning: "AI helper validation unavailable",
      redFlags: [],
      greenFlags: [],
      likelyHelpType: null,
    }));
