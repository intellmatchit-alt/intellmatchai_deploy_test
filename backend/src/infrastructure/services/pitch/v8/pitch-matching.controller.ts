...
export class PitchMatchingController {
  ...
  findMatches = async (
    ...
    try {
      ...
      const request: FindPitchMatchesRequest = {
        pitchId: String(req.params.pitchId),
    ...
    try {
      ...
      if (!text || typeof text !== "string") {
        res.status(400).json({
          ...
          error: {
            code: "MISSING_TEXT",
            message: "Provide pitch deck text.",
          },
        });
        return;
      }
  ...
  getStages = async (_req: Request, res: Response): Promise<void> => {
    res.json({
      success: true,
      data: Object.values(PitchStage).map((v) => ({
        value: v,
  ...
  getBusinessModels = async (_req: Request, res: Response): Promise<void> => {
    res.json({
      success: true,
      data: Object.values(BusinessModel).map((v) => ({ value: v, label: v })),
    });
  };

  getMatchIntents = async (_req: Request, res: Response): Promise<void> => {
    const labels: Record<MatchIntent, string> = {
      [MatchIntent.INVESTOR]: "Investor",
      [MatchIntent.ADVISOR]: "Advisor",
      [MatchIntent.STRATEGIC_PARTNER]: "Strategic Partner",
      [MatchIntent.COFOUNDER]: "Co-founder",
      [MatchIntent.CUSTOMER_BUYER]: "Customer / Buyer",
    };
    res.json({
      success: true,
      data: Object.values(MatchIntent).map((v) => ({
        value: v,
        label: labels[v],
      })),
    });
  ...
  healthCheck = async (_req: Request, res: Response): Promise<void> => {
    res.json({
      ...
      data: {
        status: "healthy",
        engine: "pitch-matching",
        version: "8.0.0",
        bands:
          "WEAK<40 | GOOD 40-59 | VERY_GOOD 60-74 | STRONG 75-89 | EXCELLENT 90-100",
        features: [
          "auth-scoped matching",
          "effectiveRankScore gating",
          "soft intent filtering",
          "multi-phase hybrid retrieval",
          "bounded AI validation",
          "per-intent scoring",
          "field-level semantic need-offer reranking",
          "supportNeededTags integration",
          "AI reasoning exposure",
          "tightened production thresholds",
        ],
      },
    });
  };
...
export const findPitchMatchesValidation = [
  ...
  body("filters.intents")
    .optional()
    .isArray()
    .custom((v: unknown[]) => {
      if (!Array.isArray(v)) return true;
      for (const i of v)
        if (!VALID_INTENTS.has(i as MatchIntent))
          throw new Error(`Invalid intent: ${i}`);
      return true;
    }),
  body("filters.stages")
    .optional()
    ...
    .custom((v: unknown[]) => {
      if (!Array.isArray(v)) return true;
      for (const i of v)
        if (!VALID_STAGES.has(i as PitchStage))
          throw new Error(`Invalid stage: ${i}`);
      return true;
    }),
  body("filters.businessModels")
    .optional()
    .isArray()
    .custom((v: unknown[]) => {
      if (!Array.isArray(v)) return true;
      for (const i of v)
        if (!VALID_MODELS.has(i as BusinessModel))
          throw new Error(`Invalid model: ${i}`);
      return true;
    }),
...
export interface PitchMatchingRouterOptions {
  prisma: any;
  config?: PitchMatchingConfig;
  authMiddleware?: any;
}

export function createPitchMatchingRoutes(
  options: PitchMatchingRouterOptions,
): Router {
  const router = Router();
  const service = createPitchMatchingService(
    options.prisma,
    options.config || DEFAULT_PITCH_CONFIG,
  );
...
export function createPitchMatchingController(
  prisma: any,
  config?: PitchMatchingConfig,
): PitchMatchingController {
  return new PitchMatchingController(
    createPitchMatchingService(prisma, config || DEFAULT_PITCH_CONFIG),
  );
}
