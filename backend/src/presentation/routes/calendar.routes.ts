...
calendarRoutes.get(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const [todayTasks, thisWeekTasks, overdueTasks, totalPendingTasks] =
        await Promise.all([
...
calendarRoutes.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const created = await meetingService.addAttendees(
        String(req.params.id),
        attendees,
        req.user.userId,
      );
