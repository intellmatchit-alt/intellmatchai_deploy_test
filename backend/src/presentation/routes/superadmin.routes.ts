...
import { authenticateSuperAdmin, requireRole } from '../middleware/superadmin.middleware.js';
  ...
  try {
    const user = await prisma.user.findUnique({
      ...
      select: {
        ...
        userSectors: { select: { sector: { select: { name: true } } }, take: 1 },
      },
    });
...
superAdminRoutes.patch('/users/:id', async (req: Request, res: Response) => {
  try {
    const allowedFields = ['fullName', 'email', 'company', 'jobTitle', 'isActive', 'status', 'emailVerified'];
    const data: any = {};
    ...
    const user = await prisma.user.update({
      where: { id: req.params.id },
      data,
      select: { id: true, email: true, fullName: true, isActive: true, status: true },
    });

    await logAudit(req.superAdmin!.adminId, 'UPDATE_USER', 'User', req.params.id, data, getIp(req));
...
superAdminRoutes.post('/users/:id/ban', async (req: Request, res: Response) => {
  try {
    ...
    await logAudit(req.superAdmin!.adminId, 'BAN_USER', 'User', req.params.id, { reason }, getIp(req));

    return res.json({ success: true, data: user, message: 'User banned successfully' });
  } catch (error: any) {
    if (error.code === 'P2025') {
      return res.status(404).json({ success: false, error: 'User not found' });
    }
 ...
 */
superAdminRoutes.post('/users/:id/unban', async (req: Request, res: Response) => {
  try {
    const user = await prisma.user.update({
      where: { id: req.params.id },
    ...
    await logAudit(req.superAdmin!.adminId, 'UNBAN_USER', 'User', req.params.id, null, getIp(req));

    return res.json({ success: true, data: user, message: 'User unbanned successfully' });
  } catch (error: any) {
    if (error.code === 'P2025') {
      return res.status(404).json({ success: false, error: 'User not found' });
    }
 ...
 */
superAdminRoutes.delete('/users/:id', requireRole('SUPER_ADMIN'), async (req: Request, res: Response) => {
  try {
    await logAudit(req.superAdmin!.adminId, 'DELETE_USER', 'User', req.params.id, null, getIp(req));
 ...
 */
superAdminRoutes.post('/users/:id/wallet', async (req: Request, res: Response) => {
  try {
    ...
    return res.json({ success: true, data: { walletId: wallet.id, balance: wallet.balance } });
  } catch (error: any) {
 ...
 */
superAdminRoutes.patch('/users/:id/plan', async (req: Request, res: Response) => {
  try {
    const { plan } = req.body;
  ...
  async (req: Request, res: Response) => {
    try {
      const {
        ...
        monthlyPrice, yearlyPrice, pointsAllocation, contactLimit,
        features, featuresAr, minSeats, maxSeats, isActive, sortOrder,
        ctaText, ctaTextAr, badgeText, badgeTextAr, badgeColor, borderColor, isHighlighted, animation,
      } = req.body;
      ...
      await logAudit(req.superAdmin!.adminId, 'CREATE_PLAN', 'PlanConfig', plan.id, { name }, getIp(req));

      return res.status(201).json({ success: true, data: plan });
    } catch (error: any) {
      if (error.code === 'P2002') {
        return res.status(409).json({ success: false, error: 'Plan name already exists' });
      }
 ...
 */
superAdminRoutes.patch('/plans/:id', requireRole('SUPER_ADMIN', 'ADMIN'), async (req: Request, res: Response) => {
  try {
    const allowedFields = [
      'displayName', 'displayNameAr', 'description', 'descriptionAr',
      'monthlyPrice', 'yearlyPrice', 'pointsAllocation', 'contactLimit',
      'features', 'featuresAr', 'minSeats', 'maxSeats', 'isActive', 'sortOrder',
      'ctaText', 'ctaTextAr', 'badgeText', 'badgeTextAr', 'badgeColor', 'borderColor', 'isHighlighted', 'animation',
    ];
    ...
    const plan = await prisma.planConfig.update({ where: { id: req.params.id }, data });

    await logAudit(req.superAdmin!.adminId, 'UPDATE_PLAN', 'PlanConfig', req.params.id, data, getIp(req));
 ...
 */
superAdminRoutes.delete('/plans/:id', requireRole('SUPER_ADMIN'), async (req: Request, res: Response) => {
  try {
    await logAudit(req.superAdmin!.adminId, 'DELETE_PLAN', 'PlanConfig', req.params.id, null, getIp(req));
 ...
 */
superAdminRoutes.get('/admins', requireRole('SUPER_ADMIN'), async (_req: Request, res: Response) => {
  try {
 ...
 */
superAdminRoutes.post('/admins', requireRole('SUPER_ADMIN'), async (req: Request, res: Response) => {
  try {
    ...
    await logAudit(req.superAdmin!.adminId, 'CREATE_ADMIN', 'SuperAdmin', admin.id, { email, role: role || 'ADMIN' }, getIp(req));
 ...
 */
superAdminRoutes.patch('/admins/:id', requireRole('SUPER_ADMIN'), async (req: Request, res: Response) => {
  try {
    ...
    await logAudit(req.superAdmin!.adminId, 'UPDATE_ADMIN', 'SuperAdmin', req.params.id, data, getIp(req));
 ...
 */
superAdminRoutes.delete('/admins/:id', requireRole('SUPER_ADMIN'), async (req: Request, res: Response) => {
  try {
    // Prevent self-delete
    ...
    await logAudit(req.superAdmin!.adminId, 'DELETE_ADMIN', 'SuperAdmin', req.params.id, null, getIp(req));
...
superAdminRoutes.post('/point-packs', async (req: Request, res: Response) => {
  try {
    ...
    if (!name || points === undefined || price === undefined) {
      return res.status(400).json({ success: false, error: 'name, points, and price are required' });
    }
 ...
 */
superAdminRoutes.patch('/point-packs/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const { name, nameAr, points, price, currency, isActive, sortOrder } = req.body;
    ...
    if (!existing) {
      return res.status(404).json({ success: false, error: 'Point pack not found' });
    }
 ...
 */
superAdminRoutes.delete('/point-packs/:id', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    return res.json({ success: true, data: { message: 'Point pack deleted' } });
  } catch (error: any) {
 ...
 */
superAdminRoutes.get('/bug-reports/stats', async (_req: Request, res: Response) => {
  try {
 ...
 */
superAdminRoutes.get('/bug-reports/:id', async (req: Request, res: Response) => {
  try {
 ...
 */
superAdminRoutes.patch('/bug-reports/:id/status', async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
superAdminRoutes.get('/affiliates/stats', authenticateSuperAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
superAdminRoutes.get('/affiliates/codes', authenticateSuperAdmin, async (req: Request, res: Response) => {
  try {
 ...
 */
superAdminRoutes.get('/affiliates/referrals', authenticateSuperAdmin, async (req: Request, res: Response) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;
 ...
 */
superAdminRoutes.get('/affiliates/payouts', authenticateSuperAdmin, async (req: Request, res: Response) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;
    const status = req.query.status as string | undefined;
 ...
 */
superAdminRoutes.get('/affiliates/:id', authenticateSuperAdmin, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
 ...
 */
superAdminRoutes.get('/affiliates', authenticateSuperAdmin, async (req: Request, res: Response) => {
  try {
    const page = parseInt(req.query.page as string) || 1;
    const limit = parseInt(req.query.limit as string) || 20;
    const status = req.query.status as string | undefined;
    const search = req.query.search as string | undefined;
    const result = await affiliateService.getAllAffiliates(page, limit, status, search);
    return res.json({ success: true, data: result });
  } catch (error: any) {
 ...
 */
superAdminRoutes.patch('/affiliates/:id/status', authenticateSuperAdmin, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    ...
    const affiliate = await affiliateService.updateAffiliateStatus(id, status, reason);
 ...
 */
superAdminRoutes.patch('/affiliates/payouts/:id', authenticateSuperAdmin, async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
...
function parseVideoUrl(url: string): { videoType: 'YOUTUBE' | 'VIMEO'; videoId: string; thumbnailUrl: string | null } | null {
  if (!url) return null;
...
superAdminRoutes.patch('/video-categories/:id', async (req, res) => {
  ...
  const category = await prisma.videoCategory.update({ where: { id: req.params.id }, data });
  res.json({ success: true, data: category });
});

superAdminRoutes.delete('/video-categories/:id', async (req, res) => {
  const videoCount = await prisma.video.count({ where: { categoryId: req.params.id } });
  if (videoCount > 0) { res.status(400).json({ success: false, error: `Cannot delete: ${videoCount} videos use this category. Reassign or delete them first.` }); return; }
...
superAdminRoutes.post('/video-tags', async (req, res) => {
  ...
  if (!name) { res.status(400).json({ success: false, error: 'Name is required' }); return; }
  const tag = await prisma.videoTag.create({ data: { name, nameAr, isActive } });
...
superAdminRoutes.patch('/video-tags/:id', async (req, res) => {
  ...
  if (nameAr !== undefined) data.nameAr = nameAr;
  if (isActive !== undefined) data.isActive = isActive;
  const tag = await prisma.videoTag.update({ where: { id: req.params.id }, data });
  res.json({ success: true, data: tag });
});

superAdminRoutes.delete('/video-tags/:id', async (req, res) => {
  const usageCount = await prisma.videoTagRelation.count({ where: { tagId: req.params.id } });
  if (usageCount > 0) { res.status(400).json({ success: false, error: `Cannot delete: ${usageCount} videos use this tag. Remove tag from videos first.` }); return; }
...
superAdminRoutes.post('/videos', async (req, res) => {
  const { title, titleAr, description, descriptionAr, videoUrl, categoryId, tagIds = [], duration, sortOrder = 0, isActive = true, isFeatured = false } = req.body;
  if (!title || !videoUrl || !categoryId) { res.status(400).json({ success: false, error: 'Title, videoUrl, and categoryId are required' }); return; }

  const parsed = parseVideoUrl(videoUrl);
  if (!parsed) { res.status(400).json({ success: false, error: 'Invalid video URL. Must be a YouTube or Vimeo link.' }); return; }

  const video = await prisma.video.create({
    data: {
      title, titleAr, description, descriptionAr, videoUrl,
      videoType: parsed.videoType, videoId: parsed.videoId, thumbnailUrl: parsed.thumbnailUrl,
      duration, categoryId, sortOrder, isActive, isFeatured,
...
superAdminRoutes.get('/videos/:id', async (req, res) => {
  ...
  if (!video) { res.status(404).json({ success: false, error: 'Video not found' }); return; }
  res.json({ success: true, data: video });
});

superAdminRoutes.patch('/videos/:id', async (req, res) => {
  const { title, titleAr, description, descriptionAr, videoUrl, categoryId, tagIds, duration, sortOrder, isActive, isFeatured } = req.body;
  const data: any = {};
  ...
  if (videoUrl !== undefined) {
    const parsed = parseVideoUrl(videoUrl);
    if (!parsed) { res.status(400).json({ success: false, error: 'Invalid video URL' }); return; }
  ...
  if (tagIds !== undefined) {
    await prisma.videoTagRelation.deleteMany({ where: { videoId: req.params.id } });
    if (tagIds.length > 0) {
