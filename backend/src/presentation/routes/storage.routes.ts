 ...
 */
storageRoutes.get('/:bucket/*', async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (!bucket || !key) {
      res.status(400).json({
        success: false,
        error: { code: 'INVALID_PATH', message: 'Bucket and key are required' },
      });
      return;
    }
    ...
    if (!available) {
      res.status(503).json({
        success: false,
        error: { code: 'STORAGE_UNAVAILABLE', message: 'Storage service is not available' },
      });
      return;
    }
 ...
 */
storageRoutes.get('/url/:bucket/*', authenticate, async (req: Request, res: Response, next: NextFunction) => {
  try {
    if (!req.user) {
      res.status(401).json({
        success: false,
    ...
    const url = await storage.getPresignedUrl(bucket, key, { expiresIn: 3600 });
    ...
    logger.error('Failed to generate presigned URL', { error, path: req.path });
    next(error);
  }
});
