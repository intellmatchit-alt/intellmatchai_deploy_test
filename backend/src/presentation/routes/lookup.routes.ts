 ...
 */
function getLookupListCacheKey(
  type: string,
  search?: string,
  parentId?: string,
  limit?: number,
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      if (!search && cacheService.isAvailable()) {
        ...
        if (cached) {
          logger.debug(`Lookup ${path} served from cache`, {
            count: cached.length,
          });
          res.status(200).json({
            success: true,
      ...
      if (path === "sectors") {
        ...
        res.status(400).json({
          ...
          error: {
            code: "INVALID_PATH",
            message: `Unknown lookup type: ${path}`,
          },
        });
        return;
      }
      ...
      logger.debug(`Lookup ${path} fetched from database`, {
        count: data.length,
        search,
      });
...
lookupRoutes.post(
  ...
  async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      logger.info(`Created new ${path?.slice(0, -1)}`, {
        id: data.id,
        name: data.name,
      });
